...
col1 = sg.Column([
    [sg.Frame(layout=[[sg.Text(size=(10, 2), justification='center', background_color='black', key='-CURRENT-')]
        , [sg.Listbox(values=list_of_projects, size=(20, 12),bind_return_key=True, key='-LIST-' )]
                      ],
...
col2 = sg.Column([
    ...
    ,
    [sg.Frame(layout=[
        [sg.TabGroup([[sg.Tab('Time Breakdown', tab1_layout)]])] # Can add more tabs here if needed
    ], title='Data:')]
    ,
...
layout: list = [
    ...
    [sg.Button('Add Project', button_color=('white', 'grey'), focus=True, key='Add'),
     sg.Button('Remove Project', button_color=('white', 'grey'), focus=True, key='Remove')],
    [sg.Button('Start', button_color=('white', 'springgreen4'), focus=True, key='Start', disabled=False),
...
window: object = sg.Window('Time Tracker', layout=layout, resizable=False)
win2_active = False
...
def timetracker(project_id, project_sub_id, project_list, activity_name, activity_sub_name, first_time, start_time,
...
def calculate_time_spent_project(project_id):
    project_table = \
        full_data_table.groupby(['project_id'], as_index=False).get_group(project_id).groupby('activity_name',
                                                                                              as_index=False)[
            'time_seconds'].sum()
...
def main_tracker():
    ...
    try:
        while True:
            ...
            if stop_threads:
                timetracker(values['-LIST-'][0], "300", project_list, new_window_name, new_window_subname, first_time,
...
while True:
    ...
    if event =='Table':
        ...
        window2.Finalize()
        selected_project = project_stats[(values['Table'][0])][0]
